...
export default function OrdersPage() {
  ...
  type filterBy = "orderstatus" | "date";
  ...
  async function fetchOrders(currentPage: number) {
    ...
    try {
      ...
      let res: Response;
      res = await fetch("/api/order/pages", {
        method: "POST",
      ...
      let pageOrders: Order[] = [];
      if (data && data.content && Array.isArray(data.content)) {
        pageOrders = data.content as Order[];
        setIsFirst(data.first || false);
        setIsLast(data.last || false);
      } else if (Array.isArray(data)) {
        pageOrders = data as Order[];
      } else if (data && Array.isArray((data as any).orders)) {
        pageOrders = (data as any).orders as Order[];
      } else {
  ...
  async function fetchFilteredOrders(currentPage: number) {
    if (!activeFilter || (activeFilter.mode !== "orderstatus" && activeFilter.mode !== "date")) {
    ...
    try {
      ...
      if (activeFilter.mode === "orderstatus") {
        payload.mode = "orderstatus";
        payload.orderStatus = activeFilter.orderStatus;
      } else {
        payload.mode = "date";
      ...
      const res = await fetch("/api/order/filter", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        credentials: "include",
      ...
      if (!res.ok) {
        console.error("Filter API error", data?.message || data);
        toast.error(data?.message || "Failed to fetch filtered orders");
        setOrders([]);
        return;
      }
  ...
  async function fetchFilteredOrdersWithFilter(pageNum: number, filter: typeof activeFilter) {
    if (!filter || (filter.mode !== "orderstatus" && filter.mode !== "date")) {
      return fetchOrders(pageNum);
    }
    ...
    try {
      ...
      if (filter.mode === "orderstatus") {
        payload.mode = "orderstatus";
        payload.orderStatus = filter.orderStatus;
      } else {
        payload.mode = "date";
  ...
  async function validateAndAddCurrentItem() {
    ...
    try {
      const res = await fetch("/api/inventory/filter", {
        ...
        headers: { "Content-Type": "application/json" },
        credentials: "include",
        body: JSON.stringify({ search: currentItem.barcode.trim(), filter: "barcode", page: 0, size: 1 }),
      });

      const data = await res.json().catch(() => null);

      if (!res.ok) {
        toast.error(data?.message || "Invalid Barcode");
        return;
      }

      const product = Array.isArray(data) ? data[0] : (data && data.content ? data.content[0] : data);

      console.log("Barcode check response:", data);
      console.log("Barcode check response pridcut :", product);
      ...
      setCurrentItem(prev => ({ ...prev, barcode: product.barcode, orderedQty: 1, sellingPrice: product.Mrp ?? prev.sellingPrice }));
  ...
  function confirmAddItem() {
    ...
    if (currentItem.orderedQty > maxQty) {
  ...
  function startEditItem(index: number) {
    ...
    // recreate pendingProduct from saved item so validation limits are present
  ...
  // helper to reset the current editing row
  ...
  // update a field on the currentItem; converts numeric fields appropriately
  ...
  function handleFilter() {
    if (filterBy === "orderstatus") {
      ...
      fetchFilteredOrdersWithFilter(1, newFilter);
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-md
          ...
          <div className="bg-white rounded-lg shadow-xl
                    ...
                    flex flex-col">
            <form onSubmit={handleSubmit} className="bg-white rounded-lg shadow-xl w-[90vw] max-w-6xl max-h-[85vh] flex flex-col">
              <div className="px-8 py-4 border-b border-gray-200
                      bg-slate-200 rounded-t-lg shrink-0">
                <h2 className="text-2xl font-bold text-slate-900">
                  Create New Order
                </h2>
                <div className="grid grid-cols-4 gap-4 text-sm font-medium text-gray-600 sticky top-0 py-2 z-10">
                  <div>
              ...
              <div className="flex-1 overflow-y-auto px-8 py-6">
                {/* Current editing row */}
                <div className="grid grid-cols-4 gap-4 items-center mt-0 border-b pb-4">
          ...
          <div className="flex justify-between items-center">
            ...
            <div className="flex items-end gap-4 mb-6">
              ...
              <CommonButton onClick={handleFilter}
                disabled={(filterBy === "date" && (!startDate || !endDate)) ||
                  (filterBy === "orderstatus" && !orderStatus.trim())}>
                Search
              </CommonButton>
